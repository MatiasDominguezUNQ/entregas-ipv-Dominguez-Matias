...
@onready var walkable_cells: Array = get_used_cells(0)
...
func astar_add_walkable_cells(walkable_cells: Array) -> Array:
	var points_array = []
...
func is_valid_node(point: Vector2) -> bool:
	return astar_node.has_point(calculate_point_index(local_to_map(to_local(point))))


func get_simple_path(world_start: Vector2, world_end: Vector2) -> Array:
	var path_start_position = local_to_map(to_local(world_start))
	
	var world_to_map_end:Vector2 = local_to_map(to_local(world_end))
	...
	if not Vector2i(path_start_position) in walkable_cells || not Vector2i(path_end_position) in walkable_cells:
		return []
	...
	for point in _point_path:
		var point_world = to_global(map_to_local(Vector2(point.x, point.y)) + _half_cell_size)
